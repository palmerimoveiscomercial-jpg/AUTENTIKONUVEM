...
check('prévia PDF persistente possui flag, validação, auditoria e rollback seguro', () => {
  ...
  assert.match(serverSource, /MEDIA_LARGE_UPLOAD_ENABLED/);
  assert.match(serverSource, /MEDIA_DRIVE_SYNC_WORKER_READY/);
  assert.match(serverSource, /function mediaAssertLargeUploadReady_/);
  assert.match(serverSource, /LARGE_UPLOAD_SAFETY_HOLD/);
  assert.match(serverSource, /api\/health\?deep=1/);
  assert.match(serverSource, /function apiVerificarProntidaoUploadGrande/);
...
check('nuvem segura, edição HTML e painel administrativo incremental', () => {
  ...
  assert.match(edgeFunction, /AUT_DRIVE_SYNC_WORKER_ENABLED/);
  assert.match(edgeFunction, /driveSyncWorker/);
  assert.match(edgeFunction, /largeUploadReady/);
