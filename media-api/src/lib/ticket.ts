...
function decodeBase64UrlStrict(value: string, code: 'TICKET_INVALID' | 'TOKEN_INVALID'): Buffer {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) {
    throw new ApiError(401, code, 'Assinatura do ticket inválida.');
  }
...
export function verifyTicket(compact: string, allowed: MediaTicket['action'][]): MediaTicket {
  ...
  const actual = decodeBase64UrlStrict(parts[1], 'TICKET_INVALID');
...
export function verifyInternal<T>(compact: string, schema: z.ZodType<T>): T {
  ...
  const expected = signature(parts[0]);
  const actual = decodeBase64UrlStrict(parts[1], 'TOKEN_INVALID');
  if (actual.length !== expected.length || !timingSafeEqual(actual, expected)) {
